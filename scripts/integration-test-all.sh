...
## default port prefixes for lsbchaind
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
DATA_DIR=$(mktemp -d -t lsbchain-datadir.XXXXX)
...
DATA_CLI_DIR=$(mktemp -d -t lsbchain-cli-datadir.XXXXX)
...
init_func() {
    ...
    "$PWD"/build/lsbchaincli config keyring-backend test --home "$DATA_CLI_DIR$i"
    "$PWD"/build/lsbchaincli keys add $KEY"$i" --home "$DATA_CLI_DIR$i" --no-backup --chain-id $CHAINID
    echo "init Lsbchain with moniker=$MONIKER and chain-id=$CHAINID"
    "$PWD"/build/ethermintd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    echo "init Lsbchaincli with chain-id=$CHAINID and config it trust-node true"
    "$PWD"/build/lsbchaincli config chain-id $CHAINID --home "$DATA_CLI_DIR$i"
    "$PWD"/build/lsbchaincli config output json --home "$DATA_CLI_DIR$i"
    "$PWD"/build/lsbchaincli config indent true --home "$DATA_CLI_DIR$i"
    "$PWD"/build/lsbchaincli config trust-node true --home "$DATA_CLI_DIR$i"
    echo "prepare genesis: Allocate genesis accounts"
    "$PWD"/build/lsbchaind add-genesis-account \
    "$("$PWD"/build/lsbchaincli keys show "$KEY$i" -a --home "$DATA_CLI_DIR$i" )" 1000000000000000000aphoton,1000000000000000000stake \
    --home "$DATA_DIR$i" --home-client "$DATA_CLI_DIR$i"
    echo "prepare genesis: Sign genesis transaction"
    "$PWD"/build/lsbchaind gentx --name $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --home-client "$DATA_CLI_DIR$i"
    echo "prepare genesis: Collect genesis tx"
    "$PWD"/build/lsbchaind collect-gentxs --home "$DATA_DIR$i"
    echo "prepare genesis: Run validate-genesis to ensure everything worked and that the genesis file is setup correctly"
    "$PWD"/build/lsbchaind validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    echo "starting lsbchain node $i in background ..."
    "$PWD"/build/lsbchaind start --pruning=nothing --rpc.unsafe --log_level "main:info,state:info,mempool:info" \
    --p2p.laddr tcp://$IP_ADDR:$NODE_P2P_PORT"$i" --address tcp://$IP_ADDR:$NODE_PORT"$i" --rpc.laddr tcp://$IP_ADDR:$NODE_RPC_PORT"$i" \
    --home "$DATA_DIR$i" \
    >"$DATA_DIR"/node"$i".log 2>&1 & disown
    
    TKCHAIN_PID=$!
    echo "started lsbchain node, pid=$TKCHAIN_PID"
    # add PID to array
    arr+=("$TKCHAIN_PID")
}
...
start_cli_func() {
    echo "starting lsbchain node $i in background ..."
    "$PWD"/build/lsbchaincli rest-server --unlock-key $KEY"$i" --chain-id $CHAINID --trace --rpc-api="web3,eth,net,personal" \
    ...
    TKCHAIN_CLI_PID=$!
    echo "started lsbchaincli node, pid=$TKCHAIN_CLI_PID"
    # add PID to array
...
if [[ -z $TEST || $TEST == "rpc" || $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test lsbchain node $HOST_RPC ..."
        if [[ $MODE == "pending" ]]; then
            sleep 150
