...
DOCKER_TAG = unstable
DOCKER_IMAGE = Khaos-Labs/lsbchain
TKCHAIN_DAEMON_BINARY = lsbchaind
TKCHAIN_CLI_BINARY = lsbchaincli
GO_MOD=GO111MODULE=on
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
LEDGER_ENABLED ?= true
HTTPS_GIT := https://github.com/Khaos-Labs/lsbchain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=lsbchain \
...
build-lsbchain-linux: go.sum
	GOOS=linux GOARCH=amd64 CGO_ENABLED=1 $(MAKE) build-lsbchain

.PHONY: build build-lsbchain build-lsbchain-linux
...
docker-build:
	...
	docker rm lsbchain || true
	# create a new container from the latest image
	docker create --name lsbchain -t -i Khaos-Labs/lsbchain:latest lsbchain
	# move the binaries to the ./build directory
	mkdir -p ./build/
	docker cp lsbchain:/usr/bin/lsbchaind ./build/ ; \
	docker cp lsbchain:/usr/bin/lsbchaincli ./build/

docker-localnet:
	docker build -f ./networks/local/lsbchainnode/Dockerfile . -t lsbchaind/node
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/Khaos-Labs/lsbchain
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(TKCHAIN_DAEMON_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\lsbchain\Z lsbchaind/node "lsbchaind testnet --v 4 -o /lsbchain --starting-ip-address 192.168.10.2 --keyring-backend=test"
	docker-compose up -d
else
	mkdir -p ./build/
	@$(MAKE) docker-localnet

	if ! [ -f build/node0/$(TKCHAIN_DAEMON_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/lsbchain:Z lsbchaind/node "lsbchaind testnet --v 4 -o /lsbchain --starting-ip-address 192.168.10.2 --keyring-backend=test"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)/build\lsbchain\Z lsbchaind/node "lsbchaind unsafe-reset-all --home=/lsbchain/node0/lsbchaind"
	@docker run --rm -v $(CURDIR)/build\lsbchain\Z lsbchaind/node "lsbchaind unsafe-reset-all --home=/lsbchain/node1/lsbchaind"
	@docker run --rm -v $(CURDIR)/build\lsbchain\Z lsbchaind/node "lsbchaind unsafe-reset-all --home=/lsbchain/node2/lsbchaind"
	@docker run --rm -v $(CURDIR)/build\lsbchain\Z lsbchaind/node "lsbchaind unsafe-reset-all --home=/lsbchain/node3/lsbchaind"
else
	@docker run --rm -v $(CURDIR)/build:/lsbchain:Z lsbchaind/node "lsbchaind unsafe-reset-all --home=/lsbchain/node0/lsbchaind"
	@docker run --rm -v $(CURDIR)/build:/lsbchain:Z lsbchaind/node "lsbchaind unsafe-reset-all --home=/lsbchain/node1/lsbchaind"
	@docker run --rm -v $(CURDIR)/build:/lsbchain:Z lsbchaind/node "lsbchaind unsafe-reset-all --home=/lsbchain/node2/lsbchaind"
...
.PHONY: build-docker-local-lsbchain localnet-start localnet-stop
